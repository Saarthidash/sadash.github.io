...
def drop_off():
    pat = pattern()
    raw_data = pd.DataFrame(pat,columns=['Date','max_n','max_r','min_n','min_r','avg_n','avg_r'])
    for k in past_n_days:
        fig, ax = plt.subplots()
        pattern_data = raw_data[-k:]
        pattern_date_ = pattern_data['Date'].tolist()
        pattern_max_n = pattern_data['max_n'].tolist()
        pattern_min_n = pattern_data['min_n'].tolist()
        pattern_avg_n = pattern_data['avg_n'].tolist()
        pattern_max_r = pattern_data['max_r'].tolist()
        pattern_min_r = pattern_data['min_r'].tolist()
        pattern_avg_r = pattern_data['avg_r'].tolist()
        plt.plot(pattern_date_,pattern_max_n,color = 'red',label = 'max for NU',linestyle='dashed')
        plt.plot(pattern_date_,pattern_max_r,color = 'red',label = 'max for RU')
        plt.plot(pattern_date_,pattern_min_n,color = 'green',label = 'min for NU',linestyle='dashed')
        plt.plot(pattern_date_,pattern_min_r,color = 'green',label = 'min for RU')
        plt.plot(pattern_date_,pattern_avg_n,color = 'black',label = 'avg for NU',linestyle='dashed')
        plt.plot(pattern_date_,pattern_avg_r,color = 'black',label = 'avg for RU')
        plt.ylim(0,10)
        plt.xticks(rotation=45,ha='right',rotation_mode='anchor',weight = 'bold')
        ax.tick_params(axis="x", labelsize=8)
        plt.tight_layout()
        ...
        plt.savefig("patterns_for"+str(k)+".png")
        #plt.show()
